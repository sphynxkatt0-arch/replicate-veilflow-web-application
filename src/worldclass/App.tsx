...
import type { ChartMode, DataQuality, MarketKey, MarketState, Timeframe } from "./types";
...
function LargePrintsPanel({ state, collapsed, onToggle }: { state: MarketState; collapsed: boolean; onToggle: () => void }) {
  const result = useMemo(() => detectLargeTrades(state.trades, state.market.key === "BTC" ? 75_000 : 25_000), [state.trades, state.market.key]);
  return (
    <section className={`vf-panel ${collapsed ? "vf-collapsed" : ""}`}>
      <header onDoubleClick={onToggle}>
...
function ReplayBar({ engine }: { engine: ReturnType<typeof useMarketEngine> }) {
  ...
  return (
    <div className={`vf-replaybar ${active ? "vf-active" : ""}`}>
      <div className="vf-replay-title"><b>{active ? "EVENT REPLAY" : "SESSION RECORDER"}</b><small>{active ? `${engine.replay.events.length.toLocaleString()} normalized events` : "Recording live candles, trades, books and metrics"}</small></div>
...
function SettingsDrawer({ settings, setSettings, onClose }: { settings: Settings; setSettings: (value: Settings | ((current: Settings) => Settings)) => void; onClose: () => void }) {
  const toggle = (key: keyof Omit<Settings, "density">) => setSettings((current) => ({ ...current, [key]: !current[key] }));
  return <div className="vf-drawer-backdrop" onMouseDown={(event: ReactMouseEvent<HTMLDivElement>) => event.target === event.currentTarget && onClose()}>
    <aside className="vf-settings-drawer">
      <header><div><small>WORKSPACE</small><h2>Display & Performance</h2></div><button type="button" onClick={onClose}>×</button></header>
      {([
      ...
      <div className="vf-disclaimer"><b>DATA INTEGRITY</b><p>Proxy instruments remain explicitly labeled. Replay uses recorded normalized events. Historical footprint completeness depends on provider trade history.</p></div>
...
export default function WorldclassApp() {
  ...
  const [settings, setSettings] = useStoredState<Settings>("vf-settings-v4", DEFAULT_SETTINGS);
  ...
  const liveOnly = engine.replay.mode === "events";
  const rootStyle = { "--vf-sidebar": `${sidebarWidth}px` } as CSSProperties;

  return (
    <div className={`vf-app vf-density-${settings.density} ${liveOnly ? "vf-replay-mode" : ""}`} style={rootStyle}>
      ...
      <div className="vf-controlbar">
        ...
        <span className="vf-separator" />
        <div className="vf-quality-strip"><QualityBadge quality={engine.state.market.quality} text={engine.state.market.productType === "spot" ? "SPOT" : "PROXY · NOT CME"} /><QualityBadge quality={engine.state.analytics.dataQuality} text={`CVD ${engine.state.analytics.dataQuality.toUpperCase()}`} /><QualityBadge quality={engine.state.book?.quality ?? "unavailable"} text={`BOOK ${(engine.state.book?.quality ?? "OFF").toUpperCase()}`} /></div>
      ...
      <main className="vf-workspace">
        <section className="vf-chart-column">
          <header className="vf-market-header">
            ...
            <div className="vf-market-meta"><span>Mark<b>{formatPrice(engine.state.metrics.markPrice, engine.state.market.priceDecimals)}</b></span><span>Oracle<b>{formatPrice(engine.state.metrics.oraclePrice, engine.state.market.priceDecimals)}</b></span><span>Funding<b>{engine.state.metrics.fundingRate === undefined ? "—" : `${(engine.state.metrics.fundingRate * 100).toFixed(4)}%`}</b></span><span>OI<b>{formatCompact(engine.state.metrics.openInterest)}</b></span><span>24h volume<b>{formatCompact(engine.state.metrics.dayVolume)} {engine.state.metrics.dayVolumeUnit === "base" ? engine.state.market.quantityUnit : "USD"}</b></span></div>
          </header>
          <MarketChart state={engine.state} mode={mode} settings={settings} replayActive={liveOnly} onFps={setFps} />
          <div className="vf-metrics">
            ...
            <MetricCard label="Session CVD" value={formatCompact(engine.state.analytics.sessionCvd, 2)} detail={engine.state.analytics.dataQuality === "live-only" ? "Live-complete; historical aggregate unavailable" : "Aggressor volume since UTC session open"} tone={(engine.state.analytics.sessionCvd ?? 0) >= 0 ? "positive" : "negative"} />
            ...
            <MetricCard label="Spread" value={formatPrice(engine.state.analytics.spread, engine.state.market.priceDecimals)} detail={`${formatPrice(engine.state.analytics.spreadBps, 2)} bps`} />
            <MetricCard label="Feed Lag" value={`${engine.state.eventLagMs.toFixed(0)} ms`} detail={`${engine.state.eventRate} events/s · ${fps} FPS`} tone={engine.state.eventLagMs > 2000 ? "warning" : undefined} />
          </div>
        </section>
      ...
      <footer className="vf-statusbar"><span><i className={`vf-status-${engine.state.status}`} />{engine.state.statusDetail}</span><span>Exchange time {engine.state.lastEventAt ? formatTime(engine.state.lastEventAt, true) : "—"}</span><span>Events {engine.state.eventRate}/s</span><span>Lag {engine.state.eventLagMs.toFixed(0)}ms</span><span>Renderer {fps} FPS</span><span>Shortcuts: 1/2/3 chart · R replay · Space play/pause</span></footer>
